...
import { Store, Cpu, Wifi, TrendingUp, RefreshCw, AlertTriangle, MapPin, CalendarCheck, Users, Plus, Check, Trash2, ChevronDown, ChevronUp } from "lucide-react";
...
export default function Dashboard() {
  ...
  const [showDistrictShops, setShowDistrictShops] = useState(false);
  ...
  return (
    <div className="space-y-8">
      ...
      {loading && !stats ? (
        ...
        <div className={`grid gap-4 sm:grid-cols-2 ${isViewer ? "lg:grid-cols-2" : "lg:grid-cols-4"}`}>
          {summaryCards.map((card) => {
            const cardContent = (
              <Card className="shadow-card transition-all duration-200 hover:-translate-y-0.5 hover:shadow-card-hover cursor-pointer">
            ...
            if ('clickable' in card && card.clickable) {
              return (
                <div key={card.title} onClick={() => setShowDistrictShops((v) => !v)}>
                  {cardContent}
                </div>
              );
            }
            ...
            return (
              <Link key={card.title} to={(card as any).link || "/shops"}>
                {cardContent}
              </Link>
            );
          })}
      ...
      {/* District Shops Section (shown when District Shops card is clicked) */}
      {isViewer && showDistrictShops && stats && stats.allShops.length > 0 && (
        <Card className="shadow-card border-primary/20">
          <CardHeader className="flex flex-row items-center justify-between">
            <div className="flex items-center gap-2">
              <MapPin className="h-5 w-5 text-primary" />
              <CardTitle className="text-base">Your District Shops</CardTitle>
            </div>
            <div className="flex items-center gap-2">
              ...
              <Button variant="ghost" size="sm" onClick={() => setShowDistrictShops(false)}>
          ...
          <CardContent>
            {filteredShops.length === 0 ? (
              <p className="text-sm text-muted-foreground text-center py-4">No shops in this district.</p>
            ) : (
                      ...
                      <div className="flex items-center justify-between rounded-lg border p-3 transition-colors hover:bg-muted/50">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-muted-foreground">{shop.district || shop.location}</p>
                          </div>
                        </div>
                        <div className="flex items-center gap-2">
      ...
      {isViewer && (
        <Card className="shadow-card">
          <CardHeader className="flex flex-row items-center justify-between">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                Plan and track your shop visits
              </p>
            </div>
          ...
          <CardContent>
            {tasks.length === 0 ? (
              <p className="text-sm text-muted-foreground text-center py-4">No tasks yet. Add one to plan your visits!</p>
            ) : (
                ...
                {tasks.map((task) => (
                  <div key={task.id} className="flex items-center gap-3 rounded-lg border p-3 transition-colors hover:bg-muted/50">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <p className="font-medium text-sm">{task.shop_name}</p>
                        {task.completed && (
                      ...
                      <div className="flex items-center gap-2 mt-0.5">
                        <span className="text-xs text-muted-foreground">
                          📅 {task.task_date === today ? "Today" : format(new Date(task.task_date + "T00:00:00"), "MMM d, yyyy")}
                        </span>
                        {task.note && (
                          <span className="text-xs text-muted-foreground truncate">• {task.note}</span>
                        )}
                      </div>
                    </div>
      ...
      {/* Shops by Crowd (for viewers) - always visible */}
      {isViewer && stats && stats.allShops.length > 0 && !showDistrictShops && (
        <Card className="shadow-card">
          <CardHeader className="flex flex-row items-center justify-between">
          ...
          <CardContent>
            <div className="space-y-3">
              {stats.allShops.slice(0, 5).map((shop) => {
                const status = getCrowdStatus(shop.crowd_count);
                return (
                  <Link key={shop.id} to="/shops">
                    <div className="flex items-center justify-between rounded-lg border p-3 transition-colors hover:bg-muted/50">
